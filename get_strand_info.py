...
def extract_strand_wise_info(pdb_name,chain):
	...
	for i in range(len(sheet_lines)) :
		...
		sheet_dict[curr_sheet]["residue_range"].append([clean_residue(residue_range[0]),clean_residue(residue_range[1])])

		sheet_dict[curr_sheet]["chain"].append(chain)

		sheet_dict[curr_sheet]["direction"].append(sense)
...
def recurse_pdbs_modified() :
	...
	for line in file :
		...
		try :
			pdb_sheet_dict = extract_strand_wise_info(pdb_name,chain)
